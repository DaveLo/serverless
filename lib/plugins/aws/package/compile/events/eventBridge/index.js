        ...
        functionObj.events.forEach((event, idx) => {
          if (event.eventBridge) {
            if (typeof event.eventBridge === 'object') {
              ...
              let busExists = false;
              if (
                'existing' in event.eventBridge &&
                event.eventBridge.existing !== undefined &&
                event.eventBridge.existing !== null
              ) {
                busExists = Boolean(event.eventBridge.existing);
              }
              ...
              if (EventBus) {
                ...
                if (typeof EventBus === 'string' && EventBus.startsWith('arn')) {
                ...
                ruleResource = {
                  'Fn::Join': [
                    ...
                    [
                      ...
                      { Ref: 'AWS::AccountId' },
                      {
                        'Fn::Join': ['/', ['rule', eventBusName, RuleName]],
                      },
                    ],
                  ],
                };
                  ...
                  eventBusResource = {
                    'Fn::Join': [
                      ...
                      [
                        ...
                        { Ref: 'AWS::AccountId' },
                        {
                          'Fn::Join': ['/', ['event-bus', eventBusName]],
                        },
                      ],
                    ],
                  };
